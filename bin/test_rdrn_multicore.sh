...
LOGFILE=tests/test_rdrn_multicore.log
...
try:
	...
	[float(line.split('=')[-1]) for line in lines[-14].split(',')]
except ValueError:
    sys.exit(1)

if running != 0 or waiting != 0 or finished != 8:
    sys.exit(2)

if not (8.25 <= turnaround <= 9.25):
    sys.exit(3)

if not (0.25 <= response <= 1.25):
    sys.exit(4)
EOF
}
...
if pqsh_test_commands | valgrind --leak-check=full bin/pqsh -p rdrn -n 2 > $LOGFILE 2> $LOGFILE.valgrind; then
    echo "Success"
else
    echo "Failure"
    exit 2
